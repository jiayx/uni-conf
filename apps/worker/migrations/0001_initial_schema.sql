...
INSERT OR IGNORE INTO groups (id, name, type, collection_ids, group_ids, builtins, enabled, sort_order, is_builtin, created_at, updated_at) VALUES
  ...
  ('builtin-crypto',    'Crypto',    'select',   '[]', '["builtin-proxy"]', '[]', 0, 7,  1, datetime('now'), datetime('now')),
  ('builtin-gaming',    'Gaming',    'select',   '[]', '["builtin-proxy"]', '[]', 0, 8,  1, datetime('now'), datetime('now')),
  ('builtin-developer', 'Developer', 'select',   '[]', '["builtin-proxy"]', '[]', 0, 9,  1, datetime('now'), datetime('now')),
