...
手动 URI 与订阅解析复用同一套协议解析逻辑；逐字段录入时，系统也会复用共享节点识别逻辑，从节点名称中识别国家 / 地区、倍率、流媒体、解锁、家宽 / 原生等标签。前端本地解析 Clash、sing-box、URI / base64 订阅时也调用同一份识别函数，所以导入预览、表单字段和最终入库字段保持一致。保存后仍写入统一的 `nodes` 数据模型，并参与国家 / 地区和标签自动节点组生成。对于只使用自建节点、没有订阅链接的用户，手动节点创建后也会确保默认导出配置、自动节点组 / 策略组和预置远程规则集已初始化。
