...
自动节点组默认开启，并在节点新增、删除、订阅刷新或设置变更后重新同步。默认只生成 `url-test` 类型，用户可以在设置中额外选择 `select` / `fallback`，也可以关闭自动生成；自动生成的国家 / 地区节点组名称默认包含旗帜 Emoji，设置中可关闭。具体启用哪些国家 / 地区、标签节点池（如 Streaming / Unlock、Native / Residential）或导入哪些订阅源自带节点组，在“节点组”的自动生成面板里完成，避免让用户手工维护节点组和策略组之间的关系。
...
节点组列表接口也会在读取时同步一次自动节点组，避免节点已经变化但用户进入节点组页时仍看到旧的国家 / 标签分组。自动生成面板需要同时展示国家 / 地区候选和标签节点池候选，且两类候选都按默认可用节点池口径排除 high-multiplier 高倍率节点；用户切换自动组类型时，国家 / 地区 key 和标签节点池 key 都要同步重建为对应的 `select` / `url-test` / `fallback` 组合。自动节点组的类型顺序、标签池定义、key 生成和 marker 解析统一放在 `@uni-conf/shared`，供后端同步、策略组出口偏好和前端表单共同复用，不在页面或 worker 里各自 hard code。
