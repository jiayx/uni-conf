...
导出页不能让用户误以为必须先新建导出配置才能导出，也不能让默认导出看起来需要选择一个固定格式。正常路径下，系统维护的默认导出已经可用；默认导出卡片直接列出全部快速导出格式，复用同一个默认 Token 和同一套范围，用户在预览、复制链接或下载时才选择 Mihomo、sing-box、Surge、节点订阅等具体格式。默认导出每个格式行都提供“预览”入口，并优先在当前导出页展开内联预览面板，展示可用性摘要、前几条 warning 和生成内容；需要完整页面时再从面板打开 `/preview?format={format}`。额外创建的是“高级导出档案”，只用于需要独立订阅 Token、独立命名或限定节点 / 规则范围的场景；高级档案可以保留一个主格式，用于主订阅链接、下载、校验、预览和兼容规则集过滤。高级导出档案弹窗默认只暴露名称和目标客户端，名称可留空并由系统按目标客户端生成，例如“默认 sing-box 配置”；弹窗顶部需要提示系统已经维护默认完整导出。节点组、策略组与出口、手动规则和远程规则集范围折叠到“高级范围设置”。这些范围不选择时就是导出全部启用内容，界面文案直接写成“导出全部启用节点组 / 策略组 / 手动规则 / 兼容规则集”，所以用户不需要理解 include 关系也能直接生成完整配置。导出配置卡片里的范围摘要按实际可导出对象计数：普通对象只统计启用项，远程规则集只统计当前目标客户端兼容且启用的规则集，避免把导出器会跳过的规则集展示成已导出。
