...
其中 `PROXY / DIRECT / REJECT` 是规则可以直接命中的基础策略；`全部节点 / 节点选择 / 自动选择` 是业务策略组可选择的出口候选。
...
空组合只保留基础出口，业务分流组由用户手动添加。无论选择哪种组合，系统都会把基础出口和可用节点组自动放入每个业务分流组，用户不需要手工维护关联关系。
...
用户第一次打开设置页、导出页或直接预览导出时，系统都会确保这个默认配置存在。这样 MVP 可以做到：

```text
粘贴订阅链接 -> 刷新订阅 -> 复制默认导出链接
```
...
其中缺失 MATCH、MATCH 不在最后属于可自动修复问题：导出器会把 MATCH 兜底放到最后，同时在预览页提示用户。缺少节点或引用不存在的策略组属于需要用户处理的问题。
