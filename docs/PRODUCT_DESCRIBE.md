...
节点组列表接口也会在读取时同步一次自动节点组，避免节点已经变化但用户进入节点组页时仍看到旧的国家 / 标签分组。自动生成面板需要同时展示国家 / 地区候选和标签节点池候选，且两类候选都按默认可用节点池口径排除 high-multiplier 高倍率节点；用户切换自动组类型时，国家 / 地区 key 和标签节点池 key 都要同步重建为对应的 `select` / `url-test` / `fallback` 组合。
