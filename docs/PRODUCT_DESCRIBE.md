...
第一版明确定位为**单管理员、自托管**的配置控制面：管理员通过一个共享 API Key 访问，订阅链接由持有随机令牌的客户端读取。它不是多租户 SaaS，不提供账号注册、团队空间、细粒度 RBAC、审计归属或租户隔离。若未来开放公共托管服务，账号体系、租户级数据隔离、密钥轮换和操作审计必须作为独立里程碑先完成，不能直接复用当前单管理员安全模型。

第一版验收标准是：新实例仅添加一个订阅或导入一份现有配置，即可得到所有已声明客户端格式的完整配置；导入前有差异预览，不支持的配置会明确保留而不静默转换；备份、恢复、部署和故障定位都有可重复的运维流程。
...
默认导出配置、自动节点组、策略组链接和预置远程规则集初始化由同一条零配置初始化链路按顺序维护：先保证默认导出配置，再同步节点组出口，再同步 `PROXY / DIRECT / REJECT` 与业务分流组成员，最后按当前组合生成和启停托管规则集。概览、设置、导入恢复、订阅源创建 / 更新 / 刷新 / 删除、节点组管理、策略组管理、导出配置管理、手动节点创建 / 更新 / 删除、手动规则 / 补充远程规则集管理和定时自动刷新都必须复用这条链路，避免某个入口只生成节点但缺少默认规则或导出配置。定时自动刷新在扫描前和处理完本轮到期订阅后都会同步一次，确保节点变化后的国家 / 标签节点组和策略组候选及时更新。

完整配置导出必须预置基础运行参数，不能要求用户先理解客户端端口和运行模式。Mihomo / Clash 默认生成 `mixed-port: 7890`、`mode: rule`、`allow-lan: false` 和 warning 日志；sing-box 默认生成 TUN 入站和 `mixed-in` 入站（`listen_port: 2080`，不自动设置系统代理）；Loon、Surge、Shadowrocket、Quantumult X、Egern 等文本客户端也要生成可直接导入的 General / runtime 段。用户选择客户端后得到的是完整配置，不是只有节点和规则的片段。
...
自动节点组默认开启，并在节点新增、删除、订阅刷新或设置变更后重新同步。默认只生成 `url-test` 类型，用户可以在设置中额外选择 `select` / `fallback`，也可以关闭自动生成；自动生成的国家 / 地区节点组名称默认包含旗帜 Emoji，设置中可关闭。具体启用哪些国家 / 地区、标签节点池（如 Streaming / Unlock、Native / Residential）或导入哪些订阅源自带节点组，在“节点组”的自动生成面板里完成，避免让用户手工维护节点组和策略组之间的关系。用户尚未显式保存启用范围时，面板默认勾选当前可识别的国家 / 地区和标签节点池候选，与后端 `auto_node_group_keys = NULL` 时“包含全部候选”的语义保持一致。

节点是否参与默认节点池、自动节点组、配置预览和最终导出，由节点自身启用状态和所属订阅源启用状态共同决定。关闭某个订阅源后，该来源下的节点即使自身仍为启用，也不会继续进入自动生成的出口候选或完整配置；用户不需要再逐个禁用节点。订阅源自带的节点组导入建议也遵守同一规则，只从启用订阅源和启用节点中生成；已经导入的订阅源节点组在后续订阅刷新时也只同步启用节点，避免用户把已关闭来源或已禁用节点重新导入为出口节点组。

关闭自动节点组只会移除按国家 / 标签生成的节点组出口，不会关闭 `PROXY / DIRECT / REJECT`、全部节点、节点选择、自动选择、故障切换，也不会停用基础默认规则集。全局节点出口继续使用系统默认可用节点池，默认排除 high-multiplier 高倍率节点。这样用户可以选择“不要自动分国家”，但仍然保留可导出的零配置骨架。
...
其中 `PROXY / DIRECT / REJECT` 是规则可以直接命中的基础策略；`全部节点 / 节点选择 / 自动选择 / 故障切换` 是业务策略组可选择的出口候选，并且都引用系统维护的“默认可用节点池”。这个节点池默认排除 high-multiplier 高倍率节点，避免高成本节点自动进入默认出口。

产品交互上，`PROXY / DIRECT / REJECT` 单独展示为“规则基础目标”，不混入业务分流策略组列表，也不要求用户创建：

- `PROXY` 是默认代理出口，系统会自动把节点选择、自动选择、故障切换、全部节点以及国家 / 标签自动节点组放进去；其中全局节点出口来自默认可用节点池。
...
业务分流策略组只展示 AI、Streaming、Telegram、GitHub、漏网之鱼等用户真正需要理解和调整的流量意图；这些分流组会自动包含基础出口和可用节点出口，用户不需要手动关联。系统内置业务分流组由当前策略组组合维护，不提供单独启停、编辑或删除入口；用户只需要调整默认出口偏好，或新增自定义业务分流组。
...
|------|----------|------------------------|
| 空组合 | 智能防污染 | 无业务分流组，只保留 `PROXY / DIRECT / REJECT` 和节点选择能力 |
| 极简模式 | 智能防污染 | 漏网之鱼 |
...
空组合只保留 `PROXY / DIRECT / REJECT` 和节点选择能力，业务分流组由用户手动添加。极简模式只暴露代理兜底，适合“不想理解分流，只要国内直连、国外代理、广告拦截”的用户。AI 优先、流媒体模式、路由器模式是更明确的入口，但底层仍然复用同一套规则集、出口排序和导出逻辑。用户切换场景组合时，系统会同步套用该组合推荐的 DNS 模式；之后仍可在设置里手动调整。
...
如果当前组合没有启用某个专用业务策略组，对应的系统托管规则集会以“系统停用”的状态创建或更新，而不是隐藏起来或回退到 `PROXY`。切换到包含该业务组的组合后，系统再自动启用并指回对应策略组；但如果是用户手动关闭了某条托管规则集，后台同步不会强行打开。`PROXY / DIRECT / REJECT` 这些基础目标始终存在，所以国内直连、广告拦截、默认代理等基础规则不受空组合影响。如果没有启用的 MATCH 规则，导出器优先用 `漏网之鱼` 作为兜底，未启用时再回退到 `PROXY`。这样空组合能保持“只保留基础能力”的语义，同时用户切换组合时不会产生悬空规则。
...
预置和内置规则集的目标分流组由系统根据当前策略组组合自动维护，用户界面在分流策略页顶部固定展示 `PROXY / DIRECT / REJECT` 和全局节点出口，分组标题使用“分流目标”表达规则命中后的去向。分流策略分组和单条规则集都提供启用 / 停用开关，不提供编辑入口；当前组合未启用的业务目标会标记为“当前组合未启用”，其系统停用规则集不能直接开启，需要先切换或调整策略组组合。需要本地补充匹配时再添加“补充规则集”。这样用户不需要逐条理解或维护默认规则到策略组的关系，也避免手动修改被下一次默认同步覆盖。

预置远程规则集还会自动写入稳定的优先级顺序，导出时按 `sort_order` 从小到大输出；导出器自身也会重新排序远程规则集，避免某个调用入口传入无序数据后破坏默认规则顺序：
...
```text
国家 / 地区识别
排除官网 / 官方地址 / 订阅地址 / 用户中心 / 流量 / 套餐 / 过期等提示节点
排除不可识别协议节点
按完整节点配置去重
...
节点组列表接口也会在读取时同步一次自动节点组，避免节点已经变化但用户进入节点组页时仍看到旧的国家 / 标签分组。自动生成面板需要同时展示国家 / 地区候选和标签节点池候选，且两类候选都按默认可用节点池口径排除 high-multiplier 高倍率节点；用户切换自动组类型时，国家 / 地区 key 和标签节点池 key 都要同步重建为对应的 `select` / `url-test` / `fallback` 组合。自动生成面板从未保存过显式 key 时，默认 key 列表必须由国家 / 地区候选和标签节点池候选共同构造，不能只默认选择国家 / 地区。自动节点组的类型顺序、标签池定义、key 生成和 marker 解析统一放在 `@uni-conf/shared`，供后端同步、策略组出口偏好和前端表单共同复用，不在页面或 worker 里各自 hard code。
...
节点组可以直接按 `tag = high-multiplier` 排除高倍率节点，或用 `tag contains multiplier:` 做倍率节点筛选。系统也会复用这些标签维护默认可用节点池和自动生成专用节点组；全局节点出口使用的默认可用节点池、自动生成的国家 / 地区节点组、Streaming Auto、Native Auto 默认都会加上 `tag not_in high-multiplier`，避免高倍率节点成为默认出口。自动组生成时也按这个默认过滤后的结果判断是否创建，所以某个国家 / 标签池如果只有高倍率节点，不会生成空节点组。手动节点组不强制排除，用户可以按自己的成本策略调整。

节点组的预览和最终导出必须使用同一条处理顺序：先按来源 / 指定节点 / 过滤条件选出候选节点，再应用节点组重命名，然后按节点组去重规则去重，最后排序。这样用户在节点组页看到的节点和最终策略组引用的节点保持一致；例如两个节点重命名成同一个名称时，按名称去重会保留重命名后的第一个节点。
...
概览页在没有可用节点时直接提供场景模板选择和订阅链接输入框。用户可以先选择默认智能、AI 优先、流媒体、路由器或空组合；这个选择只调整业务分流组合和推荐 DNS，`PROXY / DIRECT / REJECT`、全部节点、节点选择、自动选择、故障切换始终保留。用户可以一次粘贴一个或多个 URL，多个链接按换行、空格或逗号分隔；前端会拆分成多个订阅源并逐个保存，提交给后端的创建 payload 只包含 `url`，刷新、解析、节点组、默认分流策略和导出配置同步都由后端默认链路完成。批量保存时只把保存失败的 URL 留在输入框里，已保存但刷新失败的订阅源保留在系统中并显示刷新错误，方便用户改格式、User-Agent 或稍后重试。如果订阅刷新失败或没有解析出可导出节点，概览页仍保留订阅输入入口，避免用户被迫进入高级页面排查。有可用节点后概览页按常用客户端和节点订阅格式直接显示 Mihomo、Clash / OpenClash、sing-box、Loon、Surge、Shadowrocket、Quantumult X、Stash、Egern、节点订阅 Base64、节点订阅明文的订阅链接 `/sub/{default_export_token}/{canonical_filename}` 和快捷下载按钮，其中文件名由目标客户端格式统一映射，例如 `mihomo.yaml`、`clash.yaml`、`singbox.json`、`nodes.txt` 或 `nodes-raw.txt`；用户不需要先进入导出配置页才能复制链接或下载常用客户端配置。快速导出区域需要提示这些订阅链接已经包含当前启用的节点、节点组、分流策略和兼容规则集，可以直接复制到客户端使用。快捷下载由前端捕获后端 readiness 错误并显示在当前页面，不把 409 JSON 或文本错误直接打开成新标签。
概览页快捷下载复用默认导出配置的范围设置，但最终格式由下载接口路径决定；例如 `/api/export/download/singbox` 会使用默认范围生成 sing-box JSON，而不是因为默认配置名为 Mihomo 就导出 YAML。
概览页仍保留“粘贴订阅链接、系统自动生成配置、按客户端复制订阅链接”的三步解释，并在输入框上方提供场景模板选择作为低负担的第一步；三步入口只指向订阅源和导出配置页，配置预览不作为一级菜单或新手步骤入口，避免用户误以为“导出”和“预览”是两条并列流程。节点组、策略组和规则页作为高级入口保留，但不作为新用户必走步骤展示。
导出页、概览页、下载接口和公开订阅入口复用同一份导出格式文件名映射，避免 Stash、Egern、节点订阅等格式在不同入口出现不同后缀。
导出页不能让用户误以为必须先新建导出配置才能导出，也不能让默认导出看起来需要选择一个固定格式。正常路径下，系统维护的默认导出已经可用；默认导出卡片直接列出全部快速导出格式，复用同一个默认 Token 和同一套范围，用户在预览、复制链接或下载时才选择 Mihomo、sing-box、Surge、节点订阅等具体格式。默认导出每个格式行都提供“预览”入口，并打开明确的预览弹窗，展示可用性摘要、前几条 warning 和生成内容；需要完整页面时再从弹窗打开 `/preview?format={format}`。额外创建的是“高级导出档案”，只用于需要独立订阅 Token、独立命名或限定节点 / 规则范围的场景；高级档案可以保留一个主格式，用于主订阅链接、下载、校验、预览和兼容规则集过滤。高级导出档案弹窗默认只暴露名称和目标客户端，名称可留空并由系统按目标客户端生成，例如“默认 sing-box 配置”；弹窗顶部需要提示系统已经维护默认完整导出。节点组、策略组与出口、手动规则和远程规则集范围折叠到“高级范围设置”。这些范围不选择时就是导出全部启用内容，界面文案直接写成“导出全部启用节点组 / 策略组 / 手动规则 / 兼容规则集”，所以用户不需要理解 include 关系也能直接生成完整配置。导出配置卡片里的范围摘要按实际可导出对象计数：普通对象只统计启用项，远程规则集只统计当前目标客户端兼容且启用的规则集，避免把导出器会跳过的规则集展示成已导出。
导出配置卡片提供“校验”入口，复用配置预览接口生成当前客户端格式并返回同一组 warnings。卡片只显示“配置可用 / 配置需要处理”的摘要、阻塞问题 / 自动调整项 / 格式转换提示计数、预览行数和前几条具体提示；完整配置内容和全部 warning 仍可在配置预览页查看，但该页面作为导出流程里的深度排查工具保留，不出现在一级菜单。这样用户可以在复制订阅链接或下载前先确认当前配置是否可用，而不需要离开导出页。
...
其中缺失 MATCH 是默认零配置路径：导出器会自动追加兜底策略，不在预览页提示用户。只有用户显式添加了 MATCH 但没有放在最后时，预览才提示导出时会把 MATCH 放到最后。规则类型部分兼容、远程规则集格式不兼容属于可降级或可跳过问题：导出器会按客户端能力降级，或者跳过目标客户端无法承载的规则并提示用户。DNS 模式兼容性属于降级提示：Mihomo / Clash、Stash、sing-box 可以导出系统托管 DNS；其他客户端或纯节点订阅遇到智能防污染、高级 fake-ip 时会提示 DNS 字段按客户端能力降级或跳过。订阅源最近刷新失败、预览时远程规则集无法下载、规则类型完全不支持属于需要用户处理的问题，但不直接阻断已有可用图的下载；远程规则集 URL 无法作为 http(s) 下载地址、缺少节点、目标客户端完全没有可渲染节点、策略组 / 规则 / 远程规则集引用不存在的目标，下载和公开订阅会返回 409，避免给客户端一个空配置或悬空引用配置。远程规则集下载检查只针对当前客户端会实际引用的规则集，纯节点订阅不执行该网络检查。
...
新增订阅源的默认路径只要求填写订阅链接，支持一次粘贴多个链接并自动拆成多个订阅源。空状态和新增弹窗都应明确提示“只需要填写订阅链接”：保存后立即刷新订阅，自动识别节点、生成节点组、启用默认分流策略和导出配置。即使用户绕过概览页、直接从订阅源页添加链接，前端默认也只提交 `url`；后端会确保默认导出配置、自动节点组 / 策略组和预置远程规则集都已初始化。如果 API 创建请求只传 `url` 而没有传 `type`，后端也按 URL 订阅源处理并走默认刷新链路，不能要求前端替用户补齐高级字段。名称可以留空，系统会从订阅 URL 的域名自动生成；批量粘贴时名称默认留空，让每个订阅源使用自己的域名命名。概览页和订阅源页都复用同一套批量创建结果摘要：只把保存失败的 URL 留在输入框里；保存成功但刷新失败的订阅源保留在列表中，并在当前入口和订阅源卡片上显示刷新错误。订阅格式、User-Agent、刷新间隔、是否保存后立即刷新等字段都有默认值，属于高级调整项，默认折叠；只有用户实际修改高级项时，前端才把对应字段发给后端。
...
手动 URI 与订阅解析复用同一套协议解析逻辑；AnyTLS 等主流 URI 会把密码、TLS、SNI、ALPN 等协议字段同时落到原始配置和标准化解析配置，供表单回显、校验和各平台导出复用。URI scheme 自动识别从 `@uni-conf/types` 的协议注册表派生，并由 `@uni-conf/shared` 暴露给前端导入预览和后端订阅解析复用，避免 AnyTLS、ShadowTLS、WireGuard、NaiveProxy 等协议在不同入口出现白名单不一致。手动节点弹窗里的 URI placeholder 和支持列表也必须从同一注册表生成，不能再手写固定协议清单。节点订阅导出也必须按同一能力表序列化可表达的 URI，例如 SSR 节点能导出为 `ssr://`，不能出现完整配置可用但节点订阅提示不可转换的割裂状态。逐字段录入、前端本地 URI 解析和后端订阅 URI 解析都要调用 `@uni-conf/shared` 的同一套标准化策略：AnyTLS、Hysteria、Hysteria2、ShadowTLS、NaiveProxy、HTTPS 这类天然 TLS 协议默认生成 `tls: true`，Hysteria 的 `auth` 会同步成标准化密码字段，VMess 的 `aid/scy`、VLESS Reality 的 `pbk/sid`、Hysteria2 的混淆字段、AnyTLS 的指纹和 ALPN、WireGuard 的本地地址都会按协议注册表字段输出到目标平台配置；其中 AnyTLS 会同时识别 `client-fingerprint`、`clientFingerprint`、`fingerprint`、`fp` 等常见别名，WireGuard 的本地地址可以按表单和 URI 的字符串形态录入，导出 sing-box 时统一转换成 `local_address` 数组，避免表单录入、API 录入和 URI 粘贴得到不同导出结果。逐字段录入时，系统也会复用共享节点识别逻辑，从节点名称中识别国家 / 地区、倍率、流媒体、解锁、家宽 / 原生等标签，并过滤官网、流量、到期时间等提示节点。前端本地解析 Clash、sing-box、URI / base64 订阅时也调用同一份识别和提示节点过滤函数，所以导入预览、表单字段和最终入库字段保持一致。保存后仍写入统一的 `nodes` 数据模型，并参与国家 / 地区和标签自动节点组生成。对于只使用自建节点、没有订阅链接的用户，手动节点创建后也会确保默认导出配置、自动节点组 / 策略组和预置远程规则集已初始化。
...
地区标准化需要复用系统共享的国家 / 地区识别词表，把同一地区的旗帜、缩写、英文名和中文名合并成一个标准地区词，不能因为节点名同时包含 `🇭🇰`、`HK`、`Hong Kong` 而生成重复的 `香港 香港`。同一套逻辑必须同时用于前端表单预览、节点组预览、配置预览和后端导出，避免不同入口的节点名称不一致。

### 节点去重

系统默认在最终导出阶段按完整节点配置去重。即使导出范围包含默认可用节点池，或者多个节点组里引用了同一个实际节点配置，最终 `proxies` / `outbounds` 里也只保留第一条可用节点，策略组引用会同步使用去重后的导出节点名。
...
所有 `url-test`、`fallback` 和其他需要测速的出口都复用系统共享健康检查默认值：测速 URL 为 `http://www.gstatic.com/generate_204`，间隔 300 秒，容忍度 150，默认 lazy。自动节点组、内置全局节点出口、用户手动创建的测速组，以及 Mihomo / sing-box / Loon / Surge / Shadowrocket / Quantumult X / Stash / Egern 等导出器的 fallback 配置都必须从同一份默认值读取，不能各平台散落硬编码。

策略组编辑交互不暴露“嵌套哪些策略组 / 关联哪些内置出口”这类关系维护项。用户只能创建或编辑自定义业务分流组，设置名称、选择方式和测速参数；内置业务分流组由所选组合自动启用和排序，不作为可编辑对象。出口候选由系统自动维护。
...
```text
PROXY：自动包含节点选择、自动选择、故障切换、全部节点和国家 / 标签节点组，其中全局节点出口来自默认可用节点池
...
UniConf 应以内置远程规则集作为默认分流来源，让用户不用从零开始写规则。手动规则只作为用户自建补充，用于无法通过远程规则集表达的少量本地规则；系统不再提供单独的本地规则模板，也不会在初始化时创建默认手动规则。
...
/sub/abc123/nodes-raw.txt
```

导出链接、下载和配置预览都按本次请求的目标格式生成数据范围；例如默认配置是 Mihomo 时，请求 `nodes.txt` 或 `nodes-raw.txt` 仍然按节点订阅处理，只使用选中的节点组节点池，而不会套用完整配置中的策略组范围。
