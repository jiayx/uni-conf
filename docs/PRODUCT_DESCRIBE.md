...
概览页在空状态直接提供订阅链接输入框。用户粘贴 URL 后，系统保存订阅源、立即刷新、解析节点、同步自动节点组和默认分流策略；已有订阅源后概览页直接显示默认订阅链接 `/sub/{default_export_token}/mihomo.yaml`，并提供 Mihomo、Clash / OpenClash、sing-box、Loon 的快捷下载按钮，用户不需要先进入导出配置页才能复制链接或下载常用客户端配置。
