...
节点是否参与默认节点池、自动节点组、配置预览和最终导出，由节点自身启用状态和所属订阅源启用状态共同决定。关闭某个订阅源后，该来源下的节点即使自身仍为启用，也不会继续进入自动生成的出口候选或完整配置；用户不需要再逐个禁用节点。订阅源自带的节点组导入建议也遵守同一规则，只从启用订阅源和启用节点中生成，避免用户把已关闭来源重新导入为出口节点组。
