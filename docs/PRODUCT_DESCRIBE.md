...
概览页仍保留“粘贴订阅链接、系统自动生成配置、按客户端复制订阅链接”的三步解释，并在输入框上方提供场景模板选择作为低负担的第一步；三步入口分别指向订阅源、配置预览和导出配置页，节点组、策略组和规则页作为高级入口保留，但不作为新用户必走步骤展示。
...
导出配置卡片提供“校验”入口，复用配置预览接口生成当前客户端格式并返回同一组 warnings。卡片只显示“配置可用 / 配置需要处理”的摘要、阻塞问题 / 自动调整项 / 格式转换提示计数、预览行数和前几条具体提示；完整配置内容和全部 warning 仍在配置预览页查看。这样用户可以在复制订阅链接或下载前先确认当前配置是否可用，而不需要离开导出页。
