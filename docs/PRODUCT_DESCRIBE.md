...
手动 URI 与订阅解析复用同一套协议解析逻辑；AnyTLS 等主流 URI 会把密码、TLS、SNI、ALPN 等协议字段同时落到原始配置和标准化解析配置，供表单回显、校验和各平台导出复用。URI scheme 自动识别从 `@uni-conf/types` 的协议注册表派生，并由 `@uni-conf/shared` 暴露给前端导入预览和后端订阅解析复用，避免 AnyTLS、ShadowTLS、WireGuard、NaiveProxy 等协议在不同入口出现白名单不一致。手动节点弹窗里的 URI placeholder 和支持列表也必须从同一注册表生成，不能再手写固定协议清单。节点订阅导出也必须按同一能力表序列化可表达的 URI，例如 SSR 节点能导出为 `ssr://`，不能出现完整配置可用但节点订阅提示不可转换的割裂状态。逐字段录入时，系统也会复用同一套标准化策略：AnyTLS、Hysteria、Hysteria2、ShadowTLS、NaiveProxy、HTTPS 这类天然 TLS 协议默认生成 `tls: true`，Hysteria 的 `auth` 会同步成标准化密码字段，避免表单录入和 URI 粘贴得到不同导出结果。逐字段录入时，系统也会复用共享节点识别逻辑，从节点名称中识别国家 / 地区、倍率、流媒体、解锁、家宽 / 原生等标签，并过滤官网、流量、到期时间等提示节点。前端本地解析 Clash、sing-box、URI / base64 订阅时也调用同一份识别和提示节点过滤函数，所以导入预览、表单字段和最终入库字段保持一致。保存后仍写入统一的 `nodes` 数据模型，并参与国家 / 地区和标签自动节点组生成。对于只使用自建节点、没有订阅链接的用户，手动节点创建后也会确保默认导出配置、自动节点组 / 策略组和预置远程规则集已初始化。
