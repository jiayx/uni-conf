...
概览页空状态也按这个原则呈现三步：粘贴订阅链接、系统自动生成配置、复制默认导出链接；节点组、策略组和规则页作为高级入口保留，但不作为新用户必走步骤展示。
导出页和概览页复用同一份导出格式文件名映射生成订阅链接，避免 Stash、Egern、节点订阅等格式在不同入口出现不同后缀。

### 配置预览校验

生成预览时，系统会返回配置校验结果。第一版先覆盖这些问题：
...
其中缺失 MATCH、MATCH 不在最后属于可自动修复问题：导出器会把 MATCH 兜底放到最后，同时在预览页提示用户。远程规则集格式不兼容属于可跳过问题：导出器会跳过该规则集并提示用户。DNS 模式兼容性属于降级提示：Mihomo / Clash、Stash、sing-box 可以导出系统托管 DNS；其他客户端或纯节点订阅遇到智能防污染、高级 fake-ip 时会提示 DNS 字段按客户端能力降级或跳过。远程规则集 URL 无法作为 http(s) 下载地址、缺少节点或引用不存在的策略组属于需要用户处理的问题。
