...
默认导出配置、自动节点组、策略组链接和预置远程规则集初始化由同一条零配置初始化链路按顺序维护：先保证默认导出配置，再同步节点组出口，再同步 `PROXY / DIRECT / REJECT` 与业务分流组成员，最后按当前组合生成和启停托管规则集。概览、设置、导入恢复、订阅源创建 / 更新 / 刷新 / 删除、节点组管理、策略组管理、导出配置管理、手动节点创建 / 更新 / 删除、手动规则 / 补充远程规则集管理和定时自动刷新都必须复用这条链路，避免某个入口只生成节点但缺少默认规则或导出配置。
