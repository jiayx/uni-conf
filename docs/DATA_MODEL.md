...
The export UI must preserve that default-complete mental model: the modal leads with localized copy that users only need to choose a target client, hides include filters under advanced scope controls, and uses localized scope/validation summaries generated from the same effective export graph as the worker. `apps/web/src/core/export/export-i18n.test.ts` guards the translation keys for those zero-setup export labels.
