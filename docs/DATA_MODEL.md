...
Dashboard stats, export preview/download, public subscription rendering, settings reads, group reads, and remote rule set reads are initialization points for the zero-setup path. They ensure the default export config, foundation/routing policy groups, automatic node groups, and built-in remote rule sets exist before returning user-facing state or generated client content, so the first screen can show the same defaults that export will use.
