...
Dashboard stats, export config reads, export preview/download, public subscription rendering, settings reads, group reads, manual rule reads/writes, and remote rule set reads/writes are initialization points for the zero-setup path. They ensure the default export config, foundation/routing policy groups, automatic node groups, and built-in remote rule sets exist before returning user-facing state or validating user-authored rules, so the first screen and advanced rule editors use the same defaults that export will use.
