...
The export UI must preserve that default-complete mental model: normal export never requires the user to create an export config first. The page should present the system-maintained default export as already available; extra `export_configs` are advanced export profiles for separate subscription tokens or narrowed node/rule scopes. The modal leads with localized copy for that advanced-profile purpose, hides include filters under advanced scope controls, and uses localized scope/validation summaries generated from the same effective export graph as the worker. Config preview is not a separate primary navigation destination; `/preview` remains available as a deep inspection tool for generated content, while the sidebar and onboarding steps send users to export. `apps/web/src/core/export/export-i18n.test.ts` guards the translation keys for those zero-setup export labels.
