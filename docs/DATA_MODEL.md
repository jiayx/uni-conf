...
Surge and Shadowrocket INI-style local rule rendering uses the shared rule compatibility matrix before writing rules. Rules marked `unsupported` for the target client are skipped instead of being emitted verbatim, so preview compatibility warnings and generated configs stay aligned. `partial` rules can still be rendered when the exporter has a known representation or acceptable downgrade.
