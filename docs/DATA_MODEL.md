...
When the user changes the scenario template, the web app saves both `routing_policy_template` and the template's recommended `dns_mode` in the same settings update, and the worker keeps a route test for this combined write. Users can still override DNS later in Settings.
