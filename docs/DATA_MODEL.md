...
| dns_mode | TEXT? | Per-profile `compatible` \| `smart` \| `fake-ip`; `NULL` means target-format automatic for the system default or no DNS for node-only exports |
...
| name | `默认 Mihomo 配置` |
| format | `mihomo` |
| dns_mode | `NULL` (automatic by requested target format) |
...
Authenticated preview and download endpoints reuse the selected export config for scope and its optional rule-set conversion policy. The system default can render every supported route format from one token; its `dns_mode` remains `NULL` and resolves per request to `fake-ip` for Mihomo / Clash / Stash, `smart` for sing-box, `compatible` for other full-config formats, and no DNS for node-only exports. Advanced export profiles are single-format objects: preview, download, public subscription, and converted rule-set endpoints reject any target format different from the stored `format`. An advanced flexible full-config profile stores its explicit DNS choice, a compatible-only profile stores `compatible`, and a node-only profile stores `NULL`. Public subscription URLs use the token plus canonical filename to identify both the config and requested format. `rule_set_conversion_policy = NULL` follows `app_settings.rule_set_conversion_policy`; `compatible` permits safe partial conversions with diagnostics, and `strict` blocks delivery on semantic loss.
...
The export UI must preserve that default-complete mental model: normal export never requires the user to create an export config first, and the default export must not ask the user to choose a single output format or DNS mode. The page presents the system-maintained default export as already available and lists all quick-export URLs generated from the same token; each requested format automatically receives its product default DNS behavior. Each quick-export row opens a preview modal for that concrete client format while still using the default export scope; the full `/preview?format={format}` page remains available from the modal for deep inspection. The stored `format` on the default row is only its primary API format; the requested quick-export client format comes from the download route or public subscription filename. Extra `export_configs` are advanced single-format profiles for separate subscription tokens or narrowed node/rule scopes. Their editor shows a DNS selector only when the selected format has multiple supported modes, explains fixed compatible DNS without an unnecessary control, and omits DNS entirely for node-only formats. Selecting an advanced profile on the full preview page switches to its stored format; switching to another client returns to the system default.
