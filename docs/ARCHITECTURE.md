...
This is a clean-install migration policy: an existing D1 database whose migration ledger already contains `0001_initial_schema.sql` must not be upgraded in place. Export a current-version backup first, recreate the D1 database (or create a replacement binding), apply the baseline, and restore the validated backup. Backups must explicitly declare the current version; versionless payloads and earlier schema versions are rejected rather than normalized implicitly.
...
The current exporter capability revision is 17. It aligns the sing-box target with the bundled stable 1.13.13 schema: SSR is not advertised for sing-box, while WireGuard is emitted as a top-level endpoint. The converted-rule-set cache namespace is v10 and cache entries must satisfy the complete current diagnostic shape before reuse.
...
8. Reads the selected exporter's node-protocol, rule-set-container, output-kind, and managed-DNS support from `EXPORT_CLIENT_CAPABILITIES` in `@uni-conf/shared`. Backend readiness checks and the export editor use this same implementation registry, while contract tests verify that every advertised node protocol and sing-box manual-rule type has a matching serializer. The registry carries capability revision 17, aligned with the bundled sing-box 1.13.13 schema. Preview/readiness APIs return `{ id, format, revision }`, and successful authenticated downloads, public subscriptions, and token-scoped converted rule sets emit `X-UniConf-Capability-Profile: uni-conf-exporter/{format}@{revision}`. This identifies the UniConf serializer contract used for an artifact without pretending to identify the downstream client's application version.
...
10. Validates the final artifact before delivery. Format-aware validators check required sections and cross-references for Mihomo-compatible YAML, sing-box JSON, Egern YAML, Surge/Loon/Shadowrocket INI, and Quantumult X CONF. sing-box validation treats top-level endpoints as routable tags and validates WireGuard endpoint peers; an all-protocol contract also checks generated configs against the stable 1.13 schema. This catches dangling policies, missing remote provider tags, invalid remote URLs, and duplicate tags or names after rendering rather than relying only on pre-render database validation.
